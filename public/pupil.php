...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("pupil", $splitAccessPages, TRUE)) {
...
<div class="container">
    <?php
    require_once '../includes/breadcrumb_pupil.php';
    ...
    if (isset($_POST["submit"])){
        ...
        if ($validator->ValidateForm()) {
                 if (isset($_POST["submit"]) && (($_FILES["photo"]["type"] == "image/gif") || ($_FILES["photo"]["type"] == "image/jpeg") || ($_FILES["photo"]["type"] == "image/JPG") || ($_FILES["photo"]["type"] == "image/jpg") || ($_FILES["photo"]["type"] == "image/pjpeg") || ($_FILES["photo"]["type"] == "image/x-png") || ($_FILES["photo"]["type"] == "image/png")) && ($_FILES["photo"]["size"] < 1024000) && in_array($extension, $allowedExts)) {

                move_uploaded_file($_FILES["photo"]["tmp_name"], "../photos/" . $_FILES["photo"]["name"]);
                $photo->insertPic($pupil_id);
                 }
            $pupil->insertPupil();
        } else {
            echo "<div class='row'>";
...
if(mysqli_num_rows($number_part) > 0){
    ...
    echo "<ul type='square'>";
    echo "<li>Register a <strong>STUDENT</strong> by filling the form below.</li>";
    ...
    ?>">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input class="span3" type="text" name="residence_address" autocomplete="off"
                                       value="<?php
                                if (isset($residence_address)) {
                                    echo $residence_address;
                                }
    ?>">
                            </div>
                        </div>
    ...
    ?>">
                        ...
                        <div class="control-group">
                            <div class="controls">
                                ...
                                <a href="pupil.php" class="btn btn-danger">Clear</a>
                            </div>
                        </div>
                    </fieldset>
