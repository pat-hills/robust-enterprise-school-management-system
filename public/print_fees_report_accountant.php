...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("print_fees_report_accountant", $splitAccessPages, TRUE)) {
...
$pageTitle = "<strong>Total Fees Collected by " . strtoupper(htmlentities($_SESSION["accountant"])) . " between " . date("F j, Y", strtotime($_SESSION["beginning_date_accountant"])) . " and " . date("F j, Y", strtotime($_SESSION["ending_date_accountant"])) . "</strong>";
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
         ...
         <td style="text-align: center; width: 13%;"><strong>Student ID</strong></td>
         <td style="text-align: center; width: 12%;"><strong>Mode</strong></td>
         <td style="text-align: center; width: 17%;"><strong>Cheque/Draft No.</strong></td>
...
$getTotalDailyFeesCollected = $payment->getFeesCollectedBetweenSelectedDates($_SESSION["accountant"], $_SESSION["beginning_date_accountant"], $_SESSION["ending_date_accountant"]);
foreach ($getTotalDailyFeesCollected as $feesCollected) {
    if (empty($feesCollected["mode_of_payment_number"])) {
        $payment_number = "none";
    } else {
        $payment_number = $feesCollected["mode_of_payment_number"];
    }
    ...
    <tbody>
        <tr>
            ...
            <td style="width: 13%; text-align: center;">' . htmlentities($feesCollected["pupil_id"]) . '</td>
            <td style="width: 12%; text-align: left;">' . htmlentities($feesCollected["mode_of_payment"]) . '</td>
            <td style="width: 17%; text-align: left;">' . htmlentities($payment_number) . '</td>
            <td style="width: 10%; text-align: center;">' . htmlentities($feesCollected["time"]) . '</td>
            <td style="width: 30%; text-align: left;">' . htmlentities($feesCollected["fees_paid_by"]) . '</td>
            <td style="width: 13%; text-align: right;">' . number_format(htmlentities($feesCollected["amount"]), 2, ".", ",") . '</td>
        </tr>
    </tbody>';
...
$getPTADailyPaid = $payment->getPTAFeesCollectedBetweenSelectedDates($_SESSION["accountant"], $_SESSION["beginning_date_accountant"], $_SESSION["ending_date_accountant"]);
foreach ($getPTADailyPaid as $ptaPaid) {
    ...
    <tbody>
        <tr>
            <td style="text-align: center; width: 5%;">' . $i++ . '</td>
            <td style="width: 13%; text-align: center;">' . htmlentities($ptaPaid["pupil_id"]) . '</td>
            <td style="width: 12%; text-align: left;">' . htmlentities($feesCollected["mode_of_payment"]) . '</td>
            <td style="width: 17%; text-align: left;">' . htmlentities($payment_number) . '</td>
            <td style="width: 10%; text-align: center;">' . htmlentities($ptaPaid["time"]) . '</td>
            <td style="width: 30%; text-align: left;">' . htmlentities($feesCollected["fees_paid_by"]) . '</td>
...
$getPTATransactionDetails = $payment->getTotalPTAFeesCollectedBetweenSelectedDatesByClerk($_SESSION["accountant"], $_SESSION["beginning_date_accountant"], $_SESSION["ending_date_accountant"]);
$ptaAmountPaid = $getPTATransactionDetails["totalPTAFeesCollected"];
$getTotalFeesCollectedBetweenSelectedDatesByClerk = $payment->getTotalFeesCollectedBetweenSelectedDatesByClerk($_SESSION["accountant"], $_SESSION["beginning_date_accountant"], $_SESSION["ending_date_accountant"]);
...
$tbl_body_total .= '
    <tbody>
        <tr>
            ...
            <td style="width: 13%; text-align: right;"><strong>GH&cent;' . number_format(htmlentities($getTotalFeesCollectedBetweenSelectedDatesByClerk["totalFeesCollected"] + $ptaAmountPaid), 2, ".", ",") . '</strong></td>
        </tr>
    </tbody>';
