...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("edit_teacher_terminal_report", $splitAccessPages, TRUE)) {
        redirect_to("logout_access.php");
    }
}

if (!isset($_SESSION["pupil_id"])) {
    redirect_to("edit_teacher_terminal_report.php");
}
?>

<div class="container">
    <?php
    require_once '../includes/breadcrumb_terminal_report_2.php';
    ...
    if (isset($_POST["submit"])) {
        ...
        $validator->addValidation("attendance", "req", "Please, fill in the Attendance.");
        $validator->addValidation("out_of", "req", "Please, fill in the Total Attendance.");

        $validator->addValidation("attendance", "num", "Please, enter digit numbers only.");
        $validator->addValidation("out_of", "num", "Please, enter digit numbers only.");

        $validator->addValidation("conduct", "req", "Please, select Conduct.");
        $validator->addValidation("interest", "req", "Please, select Interest.");
        $validator->addValidation("attitude", "req", "Please, select Attitude.");
        ...
        if ($validator->ValidateForm()) {
            ...
            echo "<ul type = 'none'>";
            echo "<li>Please, fill in all the fields and enter digit numbers only in the Attendance fields!</li>";
            echo "</ul>";
            echo "</div>";
            echo "</div>";
            echo "</div>";
...
//                    $getMarks = $mark->getMarksDetails($_SESSION["pupil_id"]);
                    $getClassTeacherComments = $comment->getClassTeacherComments($_SESSION["pupil_id"], $institutionDetail_set["school_number"]);
...
<!--                                                    <td>
                                                        ...
                                                        $y = 0;
                                                        $prev = "";
                                                        $getClassTotalScores = $mark->getClassTotalScoresByID($getBoardingStatus["class_admitted_to"], $getStudentDetails["pupil_id"], $get_academic_term);
                                                        while ($row = mysqli_fetch_assoc($getClassTotalScores)) {
                                                            if ($prev == $row["totalScores"]) {
                                                                $position = $y;
...
//                                                        total score
                                                    ...
                                                    <td style="text-align: center;">
                                                        <!--position-->
                                                        <?php
                                                        $getTerminalReportDetails = $mark->getTerminalReportDetails($get_academic_term, $getBoardingStatus["class_admitted_to"], $getStudentDetails["pupil_id"]);
                                                        foreach ($getTerminalReportDetails as $terminalReport) {
                                                            $getSubjectPositionByStudentID = $mark->getSubjectPositionByStudentID($terminalReport["pupil_id"], $terminalReport["subject"], $terminalReport["academic_term"]);

                                                            $formatSubjectPosition = $getSubjectPositionByStudentID["position"];
                                                            if ($formatSubjectPosition == NULL) {
                                                                echo $subjectPosition = "" . "<br />";
                                                            } else {
                                                                echo $subjectPosition = ordinal($formatSubjectPosition) . "<br />";
                                                            }
                                                        }
                                                        ?>
                                                    </td>
...
//                                                        grade
                                ...
                                <table class="table table-condensed table-bordered">
                                    <tr>
                                        <td style="width: 20%;">ATTENDANCE <input type="text" name="attendance" class="span1" autocomplete="off" autofocus value="<?php
                                            if (isset($getClassTeacherComments["attendance"])) {
                                                echo $getClassTeacherComments["attendance"];
                                            } else {
                                                echo "";
                                            }
                                            ?>" /></td>
                                        <td>OUT OF <input type="text" name="out_of" class="span1" autocomplete="off" value="<?php
                                            if (isset($getTerminalSettings["total_attendance"])) {
                                                echo $getTerminalSettings["total_attendance"];
                                            }
                                            ?>" disabled />

                                            <input type="hidden" name="out_of" class="span1" autocomplete="off" value="<?php echo $getTerminalSettings["total_attendance"]; ?>" />
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td>CONDUCT</td>                                                   
                                        <td>
                                            <select class = "span5" name="conduct">
                                                <option value = "<?php
                                                if (isset($getClassTeacherComments["conduct"])) {
                                                    echo $getClassTeacherComments["conduct"];
                                                } else {
                                                    echo "--Select Conduct--";
                                                }
                                                ?>">
                                                ...
                                                <?php
                                                $getConducts = $comment->getConducts($institutionDetail_set["school_number"]);
                                                foreach ($getConducts as $value) {
                                                    ?>
                                                ...
                                                ?>
                                                                                                                                                                                    ...
                                                                                                                                                                                    <!--<input type = "text" class = "span6" name = "conduct2" placeholder = "Please, type CONDUCT here if you don't want any of the suggested CONDUCTS." />-->
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td>INTEREST</td>
                                        <td>
                                            <select class="span5" name="interest">
                                                <option value="<?php
                                                if (isset($getClassTeacherComments["interest"])) {
                                                    echo $getClassTeacherComments["interest"];
                                                } else {
                                                ...
                                                <?php
                                                $getInterests = $comment->getInterests($institutionDetail_set["school_number"]);
                                                foreach ($getInterests as $value) {
                                                    ?>
                                                    <option value = "<?php echo $value["comment"]; ?>"><?php echo $value["comment"]; ?></option>
                                                    <?php
                                                }
                                                ?>
                                                                                                                                                                                    ...
                                                                                                                                                                                    <!--<input type="text" class="span6" name="interest2" placeholder="Please, type INTEREST here if you don't want any of the suggested INTERESTS." />-->
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select class="span5" name="attitude">
                                                <option value="<?php
                                                if (isset($getClassTeacherComments["attitude"])) {
                                                    echo $getClassTeacherComments["attitude"];
                                                } else {
                                                    echo "--Select Attitude--";
                                                }
                                                ?>">
                                                            <?php
                                                            if (isset($getClassTeacherComments["attitude"])) {
                                                ...
                                                <?php
                                                $getAttitudes = $comment->getAttitudes($institutionDetail_set["school_number"]);
                                                foreach ($getAttitudes as $value) {
                                                    ?>
                                                    <option value = "<?php echo $value["comment"]; ?>"><?php echo $value["comment"]; ?></option>
                                                    <?php
                                                }
                                                ?>
                                                                                                                                                                                    ...
                                                                                                                                                                                    <!--<input type="text" class="span6" name="attitude2" placeholder="Please, type ATTITUDE here if you don't want any of the suggested ATTITUDES." />-->
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select class="span5" name="remark">
                                                <option value="<?php
                                                if (isset($getClassTeacherComments["remark"])) {
                                                    echo $getClassTeacherComments["remark"];
                                                } else {
                                                    echo "--Select Remark--";
                                                }
                                                ?>">
