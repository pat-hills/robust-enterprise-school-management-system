...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("edit_other_incomes", $splitAccessPages, TRUE)) {
...
<div class="container">
    <?php
    require_once '../includes/breadcrumb_for_edit_other_incomes.php';
    ...
    $getOtherIncomes = $bill->getOtherIncomes();
        ...
        <div class="row">
            <div class="span12">
                <form method="post">
                    ...
                    <legend style="color: #4F1ACB; font-weight: 600;">Edit Other Incomes</legend>
                                ...
                                <table class="table table-condensed table-bordered margin-left">
                                    <thead class="table-head-format">
                                        <tr style="background-color: #F5F5F5;">
                                            ...
                                            <td style="text-align: center; font-weight: 600; padding: 10px; font-size: 18px;">INCOMES</td>
                                    ...
                                    foreach ($getOtherIncomes as $otherIncome) {
                                        ?>
                                        <tr>
                                            <td style="text-align: center; padding-top: 7px;"><?php echo $i++; ?></td>

                                            <td style="text-align: left; padding-left: 5px; padding-top: 7px;">
                                                <input type="hidden" name="description" value="<?php echo htmlentities(ucwords($otherIncome["description"])); ?>">
                                                <?php echo htmlentities(ucwords($otherIncome["description"])); ?>
                                            </td>

                                            <td style="text-align: right; padding-top: 7px;"><?php echo number_format(htmlentities($otherIncome["amount"]), 2, ".", ","); ?></td>

                                            <td style="text-align: left; padding-left: 5px; padding-bottom: 5px;"><a href="edit_income_item.php?id=<?php echo urlencode(htmlentities($otherIncome["income_hash"])); ?>" class="btn btn-primary btn-block" style="padding: 2px;">Edit</a></td>
                                            <td style="text-align: left; padding-left: 5px; padding-bottom: 5px;"><a href="delete_income_item.php?id=<?php echo urlencode(htmlentities($otherIncome["income_hash"])); ?>" class="btn btn-danger btn-block" style="padding: 2px;" onclick="return confirm('Are you sure you want to delete? If YES, click OK, otherwise click CANCEL')">Delete</a></td>
                                        </tr>
                                        <?php
                                    }
                                    ?>
