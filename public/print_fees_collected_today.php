...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("print_fees_collected_today", $splitAccessPages, TRUE)) {
...
$pageTitle = "<strong>Total Fees Collected by " . strtoupper(htmlentities($get_name)) . " on " . date("F j, Y") . "</strong>";
...
$getPTATransactionDetails = $payment->getPTADailyPayments($get_name, $todayDate);
$ptaAmountPaid = $getPTATransactionDetails["ptaDailyPayment"];

$i = 1;
$getFeesCollectedTodayByClerk = $payment->getFeesCollectedTodayByClerk($get_name, $todayDate);
foreach ($getFeesCollectedTodayByClerk as $feesCollectedToday) {
...
$getPTADailyPaid = $payment->getPTADailyPaid($get_name, $todayDate);
...
$getSumOfDailyFeesCollected = $payment->getSumOfDailyFeesCollected($get_name, $todayDate);
