...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("verify_class_membership", $splitAccessPages, TRUE)) {
...
<div class="container">
    ...
    if (isset($_POST["submit"])) {
        ...
        $_SESSION["pupil_id_change_class"] = $pupil_id;

        $verifyStudent = $pupil->getPupilById($_SESSION["pupil_id_change_class"]);
        if ($verifyStudent["pupil_id"] != $_SESSION["pupil_id_change_class"]) {
            unset($_SESSION["pupil_id_change_class"]);
            redirect_to("verify_class_membership.php");
        }

        if ($validator->ValidateForm()) {
            redirect_to("change_class_membership.php");
        } else {
                        ...
                        <div class="wrapper">
                            ...
                            <div class="control-group">
                                <div class="controls">
                                    ...
                                    <a href="change_class.php" class="btn large btn-danger">Clear</a>
                                </div>
                            </div>
                        </div>
                    </fieldset>
