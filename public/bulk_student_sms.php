...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("bulk_student_sms", $splitAccessPages, TRUE)) {
        redirect_to("logout_access.php");
    }
}

if (!isset($_SESSION["bulk_sms_class_list"])) {
    $_SESSION["bulk_sms_class_list"] = "--Select Class--";
}
?>
...
<div class="container">
    ...
    if (isset($_POST["submit"])) {
        ...
        $bulk_sms_class_name = trim(ucwords(escape_value($_POST["class_name"])));
        $_SESSION["bulk_sms_class_list"] = $bulk_sms_class_name;
...
//            redirect_to("class_menu.php");
    ...
    $class_membership_set = $classMembership->getClassMembersByClassName($_SESSION["bulk_sms_class_list"]);
...
//                                        $_SESSION["bulk_sms_pupil_id"] = $getGuardianDetails["pupil_id"];
                                        ?>
                                        <tr>
                            ...
                            <a href="send_bulk_sms.php" class="btn btn-primary" style="padding: 5px; margin-left: 400px;">Click to type message to be sent to parents/guardians</a>
                        </form>
