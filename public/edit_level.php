...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("edit_level", $splitAccessPages, TRUE)) {
        redirect_to("logout_access.php");
    }
}

if (getURL()) {
    $level_url = $level->getLevelByURL(urldecode(getURL()));
}
?>
...
//        $level_id = trim(escape_value($_POST["level_id"]));

        if ($validator->ValidateForm()) {
            $level->updateRegionByURL($level_url["url_string"]);
        } else {
            echo "<div class='row'>";
        ...
        <div class="row">
            <div class="span12">
                <form class="form-horizontal" method="post">
                    <fieldset>
                        <legend style="color: #4F1ACB;"><strong>Edit level name</strong></legend>                        
                        <div class="control-group">
                            <label class="control-label" for="level_name">Level</label>
                            <div class="controls">
                                <!--<input type="hidden" name="level_id" value="<?php // echo $level_url["level_id"]; ?>" />-->
                                <input class="span2" type="text" name="level_name" value="<?php echo $level_url["level_name"]; ?>" autocomplete="off" />
                            </div>
                        </div>
                        ...
                        <div class="control-group">
                            <div class="controls">
                                ...
                                <a href="level.php" class="btn btn-danger">Cancel</a>
                            </div>
                        </div>
                    </fieldset>
