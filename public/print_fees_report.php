...
for ($i = 0; $i < count($splitAccessPages); $i++) {
    if (!in_array("print_fees_report", $splitAccessPages, TRUE)) {
...
$pageTitle = "<strong>Total Fees Collected by " . strtoupper(htmlentities($_SESSION["clerk"])) . " between " . date("F j, Y", strtotime($_SESSION["beginning_date"])) . " and " . date("F j, Y", strtotime($_SESSION["ending_date"])) . "</strong>";
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
         ...
         <td style="text-align: center; width: 13%;"><strong>Student IDs</strong></td>
...
$getTotalDailyFeesCollected = $payment->getFeesCollectedBetweenSelectedDates($_SESSION["clerk"], $_SESSION["beginning_date"], $_SESSION["ending_date"]);
foreach ($getTotalDailyFeesCollected as $feesCollected) {
    ...
    <tbody>
        <tr>
            ...
            <!--<td style="width: 9%; text-align: center;">' . htmlentities(date("d-m-Y", strtotime($feesCollected["date"]))) . '</td>-->
...
$getPTADailyPaid = $payment->getPTAFeesCollectedBetweenSelectedDates($_SESSION["clerk"], $_SESSION["beginning_date"], $_SESSION["ending_date"]);
...
$getPTATransactionDetails = $payment->getTotalPTAFeesCollectedBetweenSelectedDatesByClerk($_SESSION["clerk"], $_SESSION["beginning_date"], $_SESSION["ending_date"]);
$ptaAmountPaid = $getPTATransactionDetails["totalPTAFeesCollected"];
$getTotalFeesCollectedBetweenSelectedDatesByClerk = $payment->getTotalFeesCollectedBetweenSelectedDatesByClerk($_SESSION["clerk"], $_SESSION["beginning_date"], $_SESSION["ending_date"]);
...
$tbl_body_total .= '
    <tbody>
        <tr>
            ...
            <td style="width: 13%; text-align: right;"><strong>' . number_format(htmlentities($getTotalFeesCollectedBetweenSelectedDatesByClerk["totalFeesCollected"] + $ptaAmountPaid), 2, ".", ",") . '</strong></td>
        </tr>
    </tbody>';
