            ...
            if($amount==$total_amount_on_bill){
                ...
                $query_result = mysqli_query($connection, $query) or die(mysql_error());
                 ...
                 if($check_previous_payment==""){
                    ...
                    if($check_previous_payment <= $getAllAmountDebited){
                   ...
                   confirm_query($resultss); 
                        ...
                        $previous_payments_on_pta = $getPTAPaid;
                        
                        $pta_payment_accum = $accumulated_payments - $getAllAmountDebited;
                        
                        if($previous_payments_on_pta==""){
                    $query2i = "INSERT INTO pta_fee_payments (";
                    $query2i .= "school_number, pupil_id, amount,time, date,academic_term, entered_by";
                    $query2i .= ") VALUES (";
                    $query2i .= "'{$school_number}', '{$pupil_id}', '$pta_payment_accum', NOW(), NOW(), '{$get_academic_term}', '{$name}'";
                    $query2i .= ")";

                    $query_result2 = mysqli_query($connection, $query2i); 
                        }  else {
                            
                              if($previous_payments_on_pta <= $getPTAFeesAll){
                   ...
                   confirm_query($results_pta); 
                                 ...
                                 // $payments ->updatePTAFees($pta_payment_accum, $pupil_id);
                                   ...
                                   $query_for_update_total = "UPDATE pta_fee_payments SET amount = '$getPTAPayable' WHERE pupil_id ='$pupil_id' AND deleted='NO'";
...
//        if ($getAllTotalFeesPaid < $getAllAmountDebited) {
//            if ($amount <= $arrears) {
//                $schoolFees = $amount;
//            } elseif ($amount > $arrears) {
//                if ($amount > $getPTAArrears) {
//                    $ptaFeesPartPayment = $getPTAArrears;
//                    $schoolFees = $amount - $ptaFeesPartPayment;
//
//                    $query = "INSERT INTO pta_fee_payments (";
//                    $query .= "school_number, pupil_id, amount, date, time, academic_term, entered_by";
//                    $query .= ") VALUES (";
//                    $query .= "'{$school_number}', '{$pupil_id}', '$ptaFeesPartPayment', NOW(), NOW(), '{$get_academic_term}', '{$name}'";
//                    $query .= ")";
//
//                    $query_result = mysqli_query($connection, $query);
//                    confirm_query($query_result);
//                }
//            } elseif ($getPTAArrears == 0) {
//                $schoolFees = $amount;
//
//                $query = "INSERT INTO fee_payments (";
//                $query .= "pupil_id, school_number, amount, receipt_no, mode_of_payment, mode_of_payment_number, date, time, fees_paid_by, fees_received_by, academic_term";
//                $query .= ") VALUES (";
//                $query .= "'{$pupil_id}', '{$school_number}', '$schoolFees', '$receipt_no', '{$mode_of_payment}', '{$mode_of_payment_number}', NOW(), NOW(), '{$fees_paid_by}', '{$name}', '{$get_academic_term}'";
//                $query .= ")";
//
//                $query_result = mysqli_query($connection, $query);
//                confirm_query($query_result);
//            }
//
//            $query2 = "INSERT INTO fee_payments (";
//            $query2 .= "pupil_id, school_number, amount, receipt_no, mode_of_payment, mode_of_payment_number, date, time, fees_paid_by, fees_received_by, academic_term";
//            $query2 .= ") VALUES (";
//            $query2 .= "'{$pupil_id}', '{$school_number}', '$schoolFees', '$receipt_no', '{$mode_of_payment}', '{$mode_of_payment_number}', NOW(), NOW(), '{$fees_paid_by}', '{$name}', '{$get_academic_term}'";
//            $query2 .= ")";
//
//            $query_result2 = mysqli_query($connection, $query2);
...
//            }
//        } elseif ($getAllTotalFeesPaid > $getAllAmountDebited) {
//            $query = "INSERT INTO fee_payments (";
//            $query .= "pupil_id, school_number, amount, receipt_no, mode_of_payment, mode_of_payment_number, date, time, fees_paid_by, fees_received_by, academic_term";
//            $query .= ") VALUES (";
//            $query .= "'{$pupil_id}', '{$school_number}', '$amount', '$receipt_no', '{$mode_of_payment}', '{$mode_of_payment_number}', NOW(), NOW(), '{$fees_paid_by}', '{$name}', '{$get_academic_term}'";
...
//            if ($getPTAArrears > 0) {
//                if ($amount < $getPTAArrears) {
//                    $ptaFeesPartPayment = $amount;
//                } elseif ($amount > $getPTAArrears) {
//                    $ptaFeesPartPayment = $getPTAArrears;
//                    $schoolFees = $amount - $getPTAArrears;
//                } elseif ($getPTAArrears == 0) {
//                    $schoolFees = $amount;
//                }
//            }
...
//        $query .= "AND academic_term = '{$escape_academic_term}' ";
    ...
    public function getAllFeesPaid($id) {
        ...
        $query .= "FROM student_ledger_payment ";
        $query .= "WHERE pupil_id = '{$id}' ";
        $query .= "AND deleted = 'NO' ";
    ...
    public function getPTAPaid($id) {
     ...
     public function getCredit($id) {
        ...
        $query = "SELECT SUM(credit) AS accumulatedCredit";
        $query .= "FROM payments_credits ";
        $query .= "WHERE pupil_id = '{$id}' ";
        $query .= "AND deleted = 'NO' ";
        $query .= "GROUP BY pupil_id";
...
//        return $query_results;
    ...
    public function saveFeesPaySuccessBanner() {
        ...
        echo "<ul type='square'>";
        echo "<li><strong>FEES PAYMENT,</strong> successfully saved!</li>";
        echo "</ul>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
